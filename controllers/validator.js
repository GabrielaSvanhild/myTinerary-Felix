...
const validator =(req,res,next)=>{
    ...
    const schema = joi.object({
        ...
        password:joi.string().trim().min(8).max(20).required().pattern(new RegExp('^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,20}$')).messages({
        ...
        src:joi.string().required().pattern(new RegExp('.*\.(gif|jpe?g|bmp|png)$'
        )).messages({
